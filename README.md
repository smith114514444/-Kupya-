...
- 🧸 **自主移动**  
  库皮亚会在桌面上自由散步、闲逛，偶尔停下来发呆或对你“歪头杀”。

- 📺 **内置电视系统**  
  没事的时候它会自己打开小电视看视频！你可以把自己喜欢的**ogv**格式的视频放入 `video` 文件夹，库皮亚会随机播放。  
  *鼠标滚轮在电视画面上 = 调节音量*

- 🎨 **可自定义贴纸**  
  把贴纸图片放入 `sticker` 文件夹，库皮亚就会展示它们。  
  *鼠标滚轮 = 调整贴纸大小*  
  *鼠标侧键 = 旋转贴纸* （自定义按键即将补全）
...
- 🛠️ **完全开源 · Godot 架构**  
  基于 Godot 引擎开发，代码结构清晰，适合二次创作或学习桌面宠物实现逻辑。
...
- **无需任何额外环境** —— 下载后直接双击 `Kupya.exe` 即可运行。
...
- **视频**：把 `.mp4` 或其他 Godot 支持的视频文件放入 `video` 文件夹，电视机会自动识别并随机播放。
- **贴纸**：把图片文件（`.png` / `.jpg`）放入 `sticker` 文件夹，即可在桌宠界面生成可调节的贴纸。
